...
namespace MusteriTakipProjesi
{
    public partial class frmSatisIade : Form
    {
        ...
        void IdliYenile()
        {
            OleDbCommand cmd = new OleDbCommand("SELECT * FROM Satislar WHERE SatisID=@id", conn);
        ...
        void Kaydet()
        {
            if (txtEvrakNo.Text != "" && txtTutar.Text != "" && id == -1)
            {
                ...
                OleDbCommand cmd = new OleDbCommand("INSERT INTO Satislar(MusteriID,EvrakNo,Tarih,Vade,AciklamaID,Miktar,BirimID,Fiyat,Tutar,Tur,SonBakiye) VALUES(@MusteriID,@EvrakNo,@Tarih,@Vade,@AciklamaID,@Miktar,@BirimID,@Fiyat,@Tutar,0,@SonBakiye)", conn);
            ...
            {
                double ilkbakiye = gmt.BakiyeCek(sabitid, conn);
                OleDbCommand cmd = new OleDbCommand("UPDATE Satislar SET EvrakNo=@EvrakNo,Tarih=@Tarih,Vade=@Vade,AciklamaID=@AciklamaID,Tutar=@Tutar,Miktar=@Miktar,Fiyat=@Fiyat,SonBakiye=@SonBakiye WHERE SatisID=@satisid", conn);
                cmd.Parameters.Add("@EvrakNo", OleDbType.VarChar).Value = txtEvrakNo.Text;
                cmd.Parameters.Add("@Tarih", OleDbType.Date).Value = Convert.ToDateTime(dateTarih.DateTime.ToShortDateString());
                cmd.Parameters.Add("@Vade", OleDbType.Date).Value = Convert.ToDateTime(dateVade.DateTime.ToShortDateString());
                cmd.Parameters.Add("@AciklamaID", OleDbType.Integer).Value = Convert.ToInt16(cbAciklama.SelectedValue);
                ...
                cmd.Parameters.Add("@satisid", OleDbType.Integer).Value = id;
                if (conn.State == ConnectionState.Closed) conn.Open();
                cmd.ExecuteNonQuery();
                cmd.CommandText = "UPDATE Musteriler SET DevirBakiye=DevirBakiye- @yenibakiye WHERE SabitID=@sabit";
                cmd.Parameters.Clear();
        ...
        private void frmSatisIade_FormClosing(object sender, FormClosingEventArgs e)
        {
